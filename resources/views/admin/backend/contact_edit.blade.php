    ...
    <div class="page-content">
        ...
        <div class="card">
            <div class="card-body p-4">
                ...
                <form id="myForm" action="{{ route('contact.update') }}" method="post" class="row g-3" enctype="multipart/form-data">
                    @csrf

                    <input type="hidden" name="id" value="{{ $contatInfo->id }}">

                    <div class="form-group col-md-6">
                        <label for="input1" class="form-label"> Name</label>
                        <input type="text" name="name" class="form-control" id="name" value="{{ $contatInfo->name }}">
                    </div>
                    <div class="form-group col-md-6">
                        <label for="input1" class="form-label"> Phone Number</label>
                        <input type="number" name="phone" class="form-control" id="phone" value="{{ $contatInfo->phone }}">
                    </div>
                    <div class="form-group col-md-6">
                        <label for="input1" class="form-label"> Name</label>
                        <input type="text" name="email" class="form-control" id="email" value="{{ $contatInfo->email }}">
                    </div>
                    ...
                    <div class="form-group col-md-6">
                        ...
                        <select name="service_id" class="form-select mb-3" aria-label="Default select example">
                            ...
                            <option value="{{ $cat->id }}" {{ $cat->id == $contatInfo->service_id ? 'selected' : '' }}>{{ $cat->title }}</option>
                            @endforeach
                    ...
                    <div class="form-group col-md-12">
                        ...
                        <textarea name="massage" class="form-control" id="editor" placeholder="" rows="3">{{ $contatInfo->massage }}</textarea>
                    </div>
